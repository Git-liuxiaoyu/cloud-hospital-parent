...
@Component
@Slf4j
public class PhysicalExamRedisCacheSubscriber {
    ...
    public PhysicalExamRedisCacheSubscriber(MessageMysqlPoDao messageMysqlPoDao, ReceivedMysqlPoDao receivedMysqlPoDao, PhysicalExamMysqlPoDao physicalExamMysqlPoDao, PhysicalExamRedisPoDao physicalExamRedisPoDao, PhysicalExamRedisPoConverter physicalExamRedisPoConverter) {
     ...
     * 监听 - 队列 - division_es_q
     */
    @RabbitListener(queues = "physical_exam_redis_cache_q")
    @Transactional
    public void PhysicalExamRecordEsQueueSubscriber(Message message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag) {
        ...
        try {
            ...
            if (judgeArr[0].equals("UPDATE")) {
                handlePhysicalExamRedisCacheUpdateDelete(Integer.parseInt(judgeArr[1]));
            } else if (judgeArr[0].equals("DELETE")) {
                handlePhysicalExamRedisCacheDelete(Integer.parseInt(judgeArr[1]));
            } else {
                log.info("Subscriber_Physical_Exam_Redis_Cache 无匹配处理方法 {} -> {}", judgeArr[0], judgeArr[1]);
            }
    ...
    private void handlePhysicalExamRedisCacheDelete(Integer id) {
        /* 直接删除 */
        physicalExamRedisPoDao.deleteById(id);
    }
     ...
     * @param id
     */
    private void handlePhysicalExamRedisCacheUpdateDelete(Integer id) {
        try {
            /* 1.先查 */
            ...
            /* LOG */
            log.info("找到 PhysicalExam [{}]", physicalExamMysqlPo);
            /* 2.转为 EsPo 并存入 */
            physicalExamRedisPoDao.save(physicalExamRedisPoConverter.convert(physicalExamMysqlPo));
