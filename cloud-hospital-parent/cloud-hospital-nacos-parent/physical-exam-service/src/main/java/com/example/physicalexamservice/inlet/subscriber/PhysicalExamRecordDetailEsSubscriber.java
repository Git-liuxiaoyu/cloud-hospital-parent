...
@Component
@Slf4j
public class PhysicalExamRecordDetailEsSubscriber {
    ...
    public PhysicalExamRecordDetailEsSubscriber(MessageMysqlPoDao messageMysqlPoDao, ReceivedMysqlPoDao receivedMysqlPoDao, PhysicalExamRecordDetailMysqlPoDao physicalExamRecordDetailMysqlPoDao, PhysicalExamRecordDetailEsPoDao physicalExamRecordDetailEsPoDao, PhysicalExamMysqlPoDao physicalExamMysqlPoDao, PhysicalExamTypeMysqlPoDao physicalExamTypeMysqlPoDao, PhysicalExamRecordDetailEsPoConverter physicalExamRecordDetailEsPoConverter, WorkerServiceOpenFeignClient workerServiceOpenFeignClient) {
     ...
     * 监听 - 队列 - division_es_q
     */
    @RabbitListener(queues = "physical_exam_record_detail_es_q")
    @Transactional
    public void PhysicalExamRecordEsQueueSubscriber(Message message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag) {
        ...
        try {
            ...
            if (judgeArr[0].equals("UPDATE")) {
                handlePhysicalExamRecordDetailEsUpdateDelete(Long.parseLong(judgeArr[1]));
            } else {
                log.info("Subscriber_Physical_Exam_Record_Detail_Es 无匹配处理方法 {} -> {}", judgeArr[0], judgeArr[1]);
            }
     ...
     * @param id
     */
    private void handlePhysicalExamRecordDetailEsUpdateDelete(Long id) {
        try {
            /* 1.先查 */
            ...
            /* LOG */
            log.info("找到 PhysicalExamRecordDetail [{}]", physicalExamRecordDetailMysqlPo);
            /* 2.转为 EsPo 并存入 */
            physicalExamRecordDetailEsPoDao.save(convert(physicalExamRecordDetailMysqlPo));
        } catch (NullPointerException e) {
            log.info("没有在 MySQL 找到ID为 [{}] 的 PhysicalExamRecordDetailMysqlPo", id);
            throw new RuntimeException();
        }
    }

    private PhysicalExamRecordDetailEsPo convert(PhysicalExamRecordDetailMysqlPo physicalExamRecordDetailMysqlPo) {
        ...
        if (physicalExamRecordDetailMysqlPo.getExamdocid() != null) {
            ...
            physicalExamRecordDetailEsPo.setExamDocName(workerInfoById.getData().getName());
        }
        /* 赋值 */
        physicalExamRecordDetailEsPo.setExamid(physicalExamRecordDetailMysqlPo.getExamid());
        physicalExamRecordDetailEsPo.setExamName(physicalExamMysqlPo.getName());
        physicalExamRecordDetailEsPo.setRecordid(physicalExamRecordDetailMysqlPo.getRecordid());
        physicalExamRecordDetailEsPo.setCount(physicalExamRecordDetailMysqlPo.getCount());
        physicalExamRecordDetailEsPo.setTypeid(physicalExamRecordDetailMysqlPo.getTypeid());
        physicalExamRecordDetailEsPo.setTypeName(physicalExamTypeMysqlPo.getName());
        physicalExamRecordDetailEsPo.setId(physicalExamRecordDetailMysqlPo.getId());
        physicalExamRecordDetailEsPo.setPrice(physicalExamRecordDetailMysqlPo.getPrice());
        physicalExamRecordDetailEsPo.setStatus(physicalExamRecordDetailMysqlPo.getStatus());
        /* 返回 */
