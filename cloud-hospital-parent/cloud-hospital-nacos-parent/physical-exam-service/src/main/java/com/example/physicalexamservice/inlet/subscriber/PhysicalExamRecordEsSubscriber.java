...
@Component
@Slf4j
public class PhysicalExamRecordEsSubscriber {
    ...
    public PhysicalExamRecordEsSubscriber(MessageMysqlPoDao messageMysqlPoDao, ReceivedMysqlPoDao receivedMysqlPoDao, PhysicalExamRecordMysqlPoDao physicalExamRecordMysqlPoDao, PhysicalExamRecordEsPoDao physicalExamRecordEsPoDao, PhysicalExamRecordEsPoConverter physicalExamRecordEsPoConverter) {
     ...
     * 监听 - 队列 - division_es_q
     */
    @RabbitListener(queues = "physical_exam_record_es_q")
    @Transactional
    public void PhysicalExamRecordEsQueueSubscriber(Message message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag) {
        ...
        try {
            ...
            if (judgeArr[0].equals("UPDATE")) {
                handlePhysicalExamRecordEsUpdateDelete(Long.parseLong(judgeArr[1]));
            } else {
                log.info("Subscriber_Physical_Exam_Record_Es 无匹配处理方法 {} -> {}", judgeArr[0], judgeArr[1]);
            }
     ...
     * @param id
     */
    private void handlePhysicalExamRecordEsUpdateDelete(Long id) {
        try {
            /* 1.先查 */
            ...
            /* LOG */
            log.info("找到 PhysicalExamRecord [{}]", physicalExamRecordMysqlPo);
            /* 2.转为 EsPo 并存入 */
            physicalExamRecordEsPoDao.save(physicalExamRecordEsPoConverter.convert(physicalExamRecordMysqlPo));
